...
class Scenario(BaseScenario):
    ...
    def observation(self, agent: Agent):
        ...
        if self.env is not None:
            agent_images = torch.empty((self.world.batch_dim, self.n_agents, 11, 11, 3))
            for i in range(self.world.batch_dim):
                for j in range(self.n_agents):
                    rgb_array = self.env.render(mode="rgb_array", agent_index_focus=j, env_index=i).copy()
                    resized = torchvision.transforms.functional.resize(
                        torch.tensor(rgb_array).permute(2, 0, 1),
                        48
                    )
                    agent_images[i, j] = torchvision.transforms.functional.center_crop(resized, 11).permute(1, 2, 0)
            # import matplotlib.pyplot as plt
            # # breakpoint()
            # for i in range(self.n_agents):
            #     plt.imshow(agent_images[0, i].int())
            #     plt.show()
        print("observed")
        return torch.cat(
            [
