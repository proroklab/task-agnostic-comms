...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.collective_reward = torch.zeros((batch_dim, self.n_agents))
    ...
    def reset_world_at(self, env_index: int = None):
        ...
        if env_index is None:
            ...
            self.collective_reward = torch.zeros((self.world.batch_dim, self.n_agents), device=self.world.device)
            self._done[:] = False
        else:
            self.t[env_index] = 0
            self.collective_reward[env_index, :] = 0
            self._done[env_index] = False

    def reward(self, agent: Agent):
        agent_idx = self.world.policy_agents.index(agent)
        is_first = self.world.policy_agents.index(agent) == 0

        if is_first:
            self.t += 1

            self._done = self.t == (self.max_steps - 2)
        ...
        agent.dist_rew = torch.zeros((self.world.batch_dim, len(self.world.agents)))
        dists_bool = torch.logical_and(dists < self.desired_distance + 0.05, dists > self.desired_distance - 0.05)
        agent.dist_rew[dists_bool] = 0.1
        agent.dist_rew = agent.dist_rew.mean(-1)

        # print(agent.collision_rew, agent.dist_rew)
        self.collective_reward[:, agent_idx] += agent.collision_rew + agent.dist_rew

        sparse_rew = self.collective_reward[:, agent_idx].clone()
        sparse_rew[~self._done] = 0

        # if self._done[0].item() is True:
        #     print(sparse_rew)
...
if __name__ == "__main__":
    render_interactively(__file__, control_two_agents=True, max_steps=300)
